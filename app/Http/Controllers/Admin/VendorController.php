...
class VendorController extends Controller
{
    ...
    {
        // dd('kk');
        $data['data'] = User::where('type','3')->orderBy('id', 'ASC')->get();

        $checkslug = User::get();
        $data['stockiest'] = Stockiest::get();
    ...
    public function AssignToVandor(Request $request){
    //   dd($request->all());
    $data['stockiest_id']=$request->stockiest_id;
    ...
    public function VendorStatusDeactive($id)
    {
        $data = User::find($id);
        $data->update([
            'vendor_status'=> 0,
    ...
    public function VendorPassChange(Request $request)
    {
        $request->validate([
            'new_pass' => 'required',
        ...
        User::find($request->user_id)->update(['password'=> Hash::make($request->new_pass)]);
    
        return redirect()->back()->with('success', 'Change Password Successfully');

    }

    function checkRefer(Request $request){

        $data = User::where('referral_code',$request->code)->first();
        // return $data;
        if($data){
    ...
    public function store(Request $request)
    {
        ...
        $data = User::where('referral_code',$request->vendor_ref_code)->first();
        if($data){
            $vendor = new User();
            ...
            $vendor->refferal_vendor = $request->vendor_ref_code;

            $vendor->type = 3;
            $vendor->is_verified = 1;
            $vendor->is_available = 1;
            $vendor->stockiest_id = $request->stockiest_id;
            if ($vendor->save()) {
            ...
            return redirect()->back()->with('danger', trans('Your refer code is wrong'));
        }
